 ...
 */
 ...
 tikz_qtree(Tree, Name) :- openstream(Tree, Stream0, Name), format(Stream0, "\\Tree ", []), decodetree(Tree, Stream0), closestream(Stream0), !.

 foo(a0) :- tikz_qtree('{S (0,3)}'/['{NP (0,2)}'/['{DT (0,1)}'/[the/[]], '{N (1,2)}'/[man/[]]], '{VP (2,3)}'/['{V (2,3)}'/[jumps/[]]]]).
 ...
 openstream(Tree, Stream, Name) :-
	open(Name, write, Stream),
		...
		format(Stream, "\\tikzset{level distance=32pt}~n", []),
