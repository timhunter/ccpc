 ...
 * Modified by Ryan Musa, March 2011
 *		- adjusted header to work with Latex >2.09
 *		- easier to compile to ps, eepic and pdf don't play particularly nicely together
 *
...
openstream(Xmax, Ymax, Tree, Stream) :-
	open('ltree.tex', write, Stream),
		...
		format(Stream, "\\usepackage{epic}~n", []),
		format(Stream, "\\usepackage{eepic}~n", []),
