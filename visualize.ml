...
let save_to_file mode_note grammar_files (trees : (string Derivation.derivation_tree) list) filename =
        ...
        (* It is better to use [adjustbox] over [resizebox] because [adjustbox] only resizes the tree if it is larger than
                the specified [max width]. [resizebox] resizes all trees to the specified [width]. *)
        ...
        Printf.fprintf oc "\\makeatletter\n" ;
        Printf.fprintf oc "%% remove the stray space\n" ;
        Printf.fprintf oc "\\patchcmd{\\pgfutilsolvetwotwoleqfloat}\n" ;
