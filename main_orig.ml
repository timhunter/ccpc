...
module Deducer = functor (D : DEDUCTIVE_SYSTEM) ->
	struct
		...
		let rec consequences max_depth prims recent_items old_items chart =
...
let intersection_grammar orig_grammar symbols =
	let chart = uniques (MCFG_ParseGen_Deducer.deduce (-1) orig_grammar (MCFG_ParserGen.Prefix symbols) []) in
